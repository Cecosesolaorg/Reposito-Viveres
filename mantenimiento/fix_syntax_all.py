...
expected_bad_fragment_start = "            const totalInventoried = Object.values(inventoryData).filter(p => p.checkState === 1 && (p.qty > 0)).length;"
...
for i in range(1, 18):
    ...
    # Find the block starting with totalInventoried and ending with the return's closing brace
    # We'll just replace the lines manually for safety.
